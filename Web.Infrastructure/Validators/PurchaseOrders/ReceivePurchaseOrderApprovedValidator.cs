...
{
    public class ReceivePurchaseOrderApprovedValidator : AbstractValidator<ReceivePurchaseOrderApprovedRequest>
    {
        private readonly IGenericService Service;
        public ReceivePurchaseOrderApprovedValidator(IGenericService _Service)
        {
            Service = _Service;
            ...
            {
                order.RuleFor(x => x.ReceivingValueCurrency).GreaterThan(0).WithMessage("Required");
        ...
        async Task<bool> ReviewNameExist(ReceivePurchaseOrderApprovedRequest request, string name, CancellationToken cancellationToken)
        {
        ...
        async Task<bool> ReviewPRExist(ReceivePurchaseOrderApprovedRequest request, string pr, CancellationToken cancellationToken)
        {
        ...
        async Task<bool> ReviewPOExist(ReceivePurchaseOrderApprovedRequest request, string pr, CancellationToken cancellationToken)
        {
