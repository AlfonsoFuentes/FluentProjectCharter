    ...
    public class DeliverableGanttTaskValidator : AbstractValidator<DeliverableGanttTaskResponse>
    {
        ...
        public DeliverableGanttTaskValidator(IGenericService service)
        {
            ...
            RuleForEach(x => x.BudgetItemGanttTasks).ChildRules(order =>
            {
                ...
                .WithMessage("Budget Assigned is less than Zero");
                order.RuleFor(x => x.PercentageBudget).GreaterThan(0).When(order => order.BudgetItemId != Guid.Empty)
                .WithMessage("Percentage must be greater than zero");
                order.RuleFor(x => x.PercentageBudget).LessThanOrEqualTo(x => x.PercentageAvailable).When(order => order.BudgetItemId != Guid.Empty)
                .WithMessage(x => $"Percentage must be lower or equal than {x.PercentageAvailable}");
            });
        }
