...
namespace Web.Infrastructure.Validators.BudgetItems.Equipments
{
    ...
    public class EquipmentValidator : AbstractValidator<EquipmentResponse>
    {
        private readonly IGenericService Service;

        public EquipmentValidator(IGenericService service)
        {
            Service = service;
       ...
       .WithMessage("Reference must be defined!");
          ...
          .WithMessage("Model must be defined!");
            ...
            //RuleFor(x => x.ShowDetails)
            // .MustAsync(ReviewIfTemplateExist)
            // .When(x => x.ShowDetails && !x.ShowProvisionalTag)
            //.WithMessage(x => $"Equipment Template already exist");
        }
        
        async Task<bool> ReviewIfNameExist(EquipmentResponse request, string name, CancellationToken cancellationToken)
        {
        ...
        async Task<bool> ReviewIfTagExist(EquipmentResponse request, string tag, CancellationToken cancellationToken)
        {
