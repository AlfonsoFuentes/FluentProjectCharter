...
namespace FluentWeb.Pages.ScopeManagements.LearnedLessons;
#nullable disable
public partial class LearnedLessonTable
{
    [Parameter]
    ...
    Func<LearnedLessonResponse, bool> fiterexpresion => x =>
       x.Name.Contains(nameFilter, StringComparison.CurrentCultureIgnoreCase);
    public List<LearnedLessonResponse> FilteredItems => Items.Count == 0 ? new() : Items.Where(fiterexpresion).ToList();

    LearnedLessonResponse CreateRow = null!;
    LearnedLessonResponse EditRow = null!;
    ...
    bool DisableSaveButton(LearnedLessonResponse model)
    {
    ...
    private void HandleRowClick(FluentDataGridRow<LearnedLessonResponse> row)
    {
    ...
    private void HandleRowDoubleClick(FluentDataGridRow<LearnedLessonResponse> row)
    {
    ...
    async Task Update(LearnedLessonResponse model)
    {
