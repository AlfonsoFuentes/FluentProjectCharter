...
namespace Shared.Models.Deliverables.Responses
{
    ...
    public class DeliverableResponseList
    {
        ...
        public DeliverableResponse AddDeliverableResponse(Guid projectId, DeliverableResponse? selectedRow)
        {
            ...
            if (selectedRow == null)
            {
                ...
                newDeliverable.Name = $"New Deliverable-{lastOrder}";
                newDeliverable.Order = lastOrder;
                newDeliverable.LabelOrder = lastLabelOrder;
            ...
            else
            {
                ...
                if (item != null)
                {
                    if (item.SubDeliverables.Any())
                    {
                        ...
                        newDeliverable.Order = 1;
                        newDeliverable.ParentDeliverableId = item.Id;
                        newDeliverable.Name = $"SubDeliverable-1";
                        item.OrderedSubDeliverables.ForEach(x => { x.Order += 1; });
                        item.SubDeliverables.Add(newDeliverable);
                        Calculate();
                    ...
                    else
                    {
                        ...
                        {
                            newDeliverable.Order = item.Order + 1;
                            newDeliverable.ParentDeliverableId = parentedItem.Id;
                            newDeliverable.Name = $"SubDeliverable-{item.Order + 1}";

                            var subdeliverablesparent = parentedItem.OrderedSubDeliverables.Where(x => x.Order > item.Order).ToList();
                            subdeliverablesparent.ForEach(x => { x.Order += 1; });
                            parentedItem.SubDeliverables.Add(newDeliverable);
                            Calculate();
                            return newDeliverable;
                        }
        ...
        public void RemoveDeliverableResponse(DeliverableResponse deliverable)
        {
            ...
            {
                parent.SubDeliverables.Remove(deliverable);
            }
            Calculate();
        ...
        public void MoveUp(DeliverableResponse selectedRow)
        {
            ...
            if (currentParent != null)
            {
                ...
                var list = currentParent.OrderedSubDeliverables;
        ...
        public void MoveDown(DeliverableResponse selectedRow)
        {
            ...
            var list = parent?.OrderedSubDeliverables ?? Items;
        ...
        public DeliverableResponseStyles2 Styles { get; set; } = new();
        public DeliverableResponseStyles Styles3 { get; set; } = new();
        public void CalculateColumnWidths()
        {
            List<DeliverableResponse> items = FlatOrderedItems;
            // Calcular el ancho máximo para cada columna basado en el contenido
            Styles3.IdWidth = Math.Max(Styles3.IdWidth, GetMaxContentWidth(items.Select(t => t.LabelOrder.ToString())));
            Styles3.WbsWidth = Math.Max(Styles3.WbsWidth, GetMaxContentWidth(items.Select(t => t.WBS)));
            Styles3.NameWidth = Math.Min(Styles3.NameWidth, GetMaxContentWidth(items.Select(t => t.Name)));
            Styles3.StartDateWidth = Math.Max(Styles3.StartDateWidth, GetMaxContentWidth(items.Select(t => t.StartDateValue.ToShortDateString().ToString())));
            Styles3.EndDateWidth = Math.Max(Styles3.EndDateWidth, GetMaxContentWidth(items.Select(t => t.EndDateValue.ToShortDateString().ToString())));
            Styles3.DurationWidth = Math.Max(Styles3.DurationWidth, GetMaxContentWidth(items.Select(t => t.Duration!)));
            Styles3.DependencesWidth = Math.Max(Styles3.DependencesWidth, GetMaxContentWidth(items.Select(t => t.sDependences)));
            Styles3.DependencyTypeWidth = Math.Max(Styles3.DependencyTypeWidth, GetMaxContentWidth(items.Select(t => t.DependencyType.Name)));
            Styles3.LagWidth = Math.Max(Styles3.LagWidth, GetMaxContentWidth(items.Select(t => t.Lag!)));
        }
        public void CalculateColumnWidths2()
        {
            List<DeliverableResponse> items = FlatOrderedItems;
            // Calcular el ancho máximo para cada columna basado en el contenido
            Styles3.IdWidth = Math.Max(Styles3.IdWidth, GetMaxContentWidth(items.Select(t => t.LabelOrder.ToString())));
        ...
        private double GetMaxContentWidth(IEnumerable<string> contents)
        {
            // Calcular el ancho máximo basado en el contenido (simulación simple)
            if (contents == null || contents.Count() == 0 || contents.All(x => x == string.Empty)) return 0;
            return contents
                .Where(c => !string.IsNullOrEmpty(c))
                .Max(c => c.Length * 8); // Aproximación: 1 carácter ≈ 8 píxeles
        }
