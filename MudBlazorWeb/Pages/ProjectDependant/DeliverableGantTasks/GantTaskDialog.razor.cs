...
public partial class GantTaskDialog
{
    ...
    [Parameter]
    public DeliverableGanttTaskResponseList GanttTaskItemResponse { get; set; } = new();
    ...
    void RecreateGanttTaskItems()
    {
        ...
        foreach (var row in GanttTaskItemResponse.Items)
        {
            ...
            else if (GanttTaskItemResponse.IsValidNewDependency(Model, row) == string.Empty)
            {
                AddnewItem = true;
                GanttTaskItems.Add(row);
            }
    ...
    void RecreateBudgetItems()
    {
        ...
        foreach (var row in BudgetItemResponse.Items)
        {
