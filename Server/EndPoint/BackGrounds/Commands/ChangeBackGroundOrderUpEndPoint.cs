...
{
    public static class ChangeBackGroundOrderUpEndPoint
    {
        public class EndPoint : IEndPoint
        {
            public void MapEndPoint(IEndpointRouteBuilder app)
            {
                app.MapPost(StaticClass.BackGrounds.EndPoint.UpdateUp, async (ChangeBackGroundOrderUpRequest Data, IRepository Repository) =>
                {
                    ...
                    Expression<Func<BackGround, bool>> Criteria = x => x.ProjectId == Data.ProjectId && x.Order == row.Order - 1;
